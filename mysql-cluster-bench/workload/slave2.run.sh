...
then
  SOCK="--mysql-socket=$MYSQL_SLAVE2_SOCK"
fi


for (( c=1; c<=$scaleupto; c*=2 ))
do
  $slave2cores sysbench --threads=$c --time=$time_per_tc --rate=0 --report-interval=1 \
           --db-driver=mysql --rand-type=uniform \
           --warmup-time=$warmup_per_tc \
           --mysql-host=$MYSQL_HOST --mysql-port=$MYSQL_SLAVE2_PORT $SOCK \
           --mysql-db=$MYSQL_DB \
