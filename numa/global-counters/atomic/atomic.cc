...
public:
  ...
  const uint64_t get() { return (m_value.load(std::memory_order_relaxed)); }
  ...
  void inc(uint64_t incr) {
    m_value.fetch_add(incr, std::memory_order_relaxed);
  }

  void dec(uint64_t decr) {
    m_value.fetch_sub(decr, std::memory_order_relaxed);
  }
