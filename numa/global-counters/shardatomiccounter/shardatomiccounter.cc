...
// static size_t shard_slot_selector() { return (rand() % k_cores); }
static size_t shard_slot_selector() { return (0); }
...
public:
  ...
  void inc(uint32_t idx) { m_values[idx]->inc(idx); }
  void dec(uint32_t idx) { m_values[idx]->dec(idx); }
  void inc(uint32_t idx, uint64_t incr) { m_values[idx]->inc(incr, idx); }
  void dec(uint32_t idx, uint64_t decr) { m_values[idx]->dec(decr, idx); }
...
public:
  ...
  void inc(size_t idx) { m_value.inc(idx); }
  void dec(size_t idx) { m_value.dec(idx); }
  void inc(uint64_t incr, size_t idx) { m_value.inc(incr, idx); }
  void dec(uint64_t decr, size_t idx) { m_value.dec(decr, idx); }
...
public:
  ...
  void process_data_block() {
    ...
    m_data_blocks[data_block1]->inc(data_block1);
    m_data_blocks[data_block2]->inc(data_block2);

    /* write align access */
    memset(m_data_blocks[data_block3]->data(), rand() % 255, k_data_size);
    m_data_blocks[data_block3]->inc(data_block3);
    ...
    crc32(crcval, (const unsigned char *)m_data_blocks[data_block4]->data(),
          k_data_size);
    m_data_blocks[data_block4]->inc(data_block4);
