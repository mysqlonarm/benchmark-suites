...
/* Each counter is an independent atomic unit. */
class Counter {
public:
  Counter() : m_value(0), m_lock() { pthread_mutex_init(&m_lock, nullptr); }
  ~Counter() { pthread_mutex_destroy(&m_lock); }
...
public:
  CounterBlock(size_t num_of_counters) : m_num_of_counters(num_of_counters) {
    m_values = new Counter *[m_num_of_counters];
    for (uint32_t i = 0; i < m_num_of_counters; ++i) {
      m_values[i] = new Counter();
    }
  }
...
/* PayLoad allocate counter blocks and data blocks. */
class PayLoad {
public:
  PayLoad(size_t num_of_counters, size_t num_of_counter_blocks,
    ...
    /* Counter and Data Blocks are interleaved */
  ...
  void process_data_block() {
    /* Process 4 random blocks per invocation */
