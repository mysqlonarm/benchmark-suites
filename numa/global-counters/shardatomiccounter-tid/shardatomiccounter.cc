...
thread_local size_t _thread_id = 0;
...
public:
  ...
  void inc(uint32_t idx) { m_values[idx]->inc(_thread_id); }
  void dec(uint32_t idx) { m_values[idx]->dec(_thread_id); }
  void inc(uint32_t idx, uint64_t incr) {
    m_values[idx]->inc(incr, _thread_id);
  }
  void dec(uint32_t idx, uint64_t decr) {
    m_values[idx]->dec(decr, _thread_id);
  }
...
public:
  ...
  void inc() { m_value.inc(_thread_id); }
  void dec() { m_value.dec(_thread_id); }
  void inc(uint64_t incr) { m_value.inc(incr, _thread_id); }
  void dec(uint64_t decr) { m_value.dec(decr, _thread_id); }
