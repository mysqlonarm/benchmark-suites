...
static size_t shard_slot_selector() {
  return (numa_node_of_cpu(sched_getcpu()));
}

/* Each shard is cacheline aligned */
template <typename Type> struct Shard {
  char m_pad[k_cacheline_size - sizeof(std::atomic<Type>)];
  std::atomic<Type> m_value{};
};

template <size_t N = k_numa_nodes> class Counter {
public:
  Counter() : m_shards() {}

  /* There are N shards slots but if caller fail to provide
  we always use 0th slot. */
  void inc(size_t id) { inc(1, id); }
  void dec(size_t id) { dec(1, id); }

  void inc(uint64_t incr, size_t id) {
    m_shards[id % m_shards.size()].m_value.fetch_add(incr,
                                                     std::memory_order_relaxed);
  }

  void dec(uint64_t decr, size_t id) {
    m_shards[id % m_shards.size()].m_value.fetch_sub(decr,
                                                     std::memory_order_relaxed);
  }
  ...
  const uint64_t get() {
    ...
    for (size_t i = 0; i < m_shards.size(); ++i) {
      total += m_shards[i].m_value.load(std::memory_order_relaxed);
    }
    return total;
  }
  ...
  void set(uint64_t val) {
    reset();
    size_t id = rand() % m_shards.size();
    m_shards[id].m_value.store(val, std::memory_order_relaxed);
  }

  void reset() {
    for (size_t i = 0; i < m_shards.size(); ++i) {
      m_shards[i].m_value.store(0);
    }
  }

private:
  std::array<Shard<uint64_t>, N> m_shards;
};
...
public:
  CounterBlock(size_t num_of_counters) : m_num_of_counters(num_of_counters) {
    m_values = new Counter<k_numa_nodes> *[m_num_of_counters];
    for (uint32_t i = 0; i < m_num_of_counters; ++i) {
      m_values[i] = new Counter<k_numa_nodes>();
    }
  }
  ...
  void inc(uint32_t idx) { m_values[idx]->inc(shard_slot_selector()); }
  void dec(uint32_t idx) { m_values[idx]->dec(shard_slot_selector()); }
  void inc(uint32_t idx, uint64_t incr) {
    m_values[idx]->inc(incr, shard_slot_selector());
  }
  void dec(uint32_t idx, uint64_t decr) {
    m_values[idx]->dec(decr, shard_slot_selector());
  }
...
public:
  ...
  void inc() { m_value.inc(shard_slot_selector()); }
  void dec() { m_value.dec(shard_slot_selector()); }
  void inc(uint64_t incr) { m_value.inc(incr, shard_slot_selector()); }
  void dec(uint64_t decr) { m_value.dec(decr, shard_slot_selector()); }
